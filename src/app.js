...
class REVERSApp {
  ...
  _buildDOM() {
    ...
    document.getElementById('app').innerHTML = `
      ...
      <div class="modal" id="groupSettingsModal"><h3>⚙️ Настройки группы</h3><div id="groupSettingsContent"></div><button id="closeGroupSettingsBtn" class="secondary">Закрыть</button></div>
      ...
      <div class="modal" id="aboutModal"><h3>ℹ️ О REVERS Messenger</h3><div style="color:#EFEFEF; font-size:0.85rem; line-height:1.6;"><p><strong>REVERS v1.3</strong></p><p>P2P мессенджер с полной анонимностью.</p><p>• Никаких серверов • Сквозное шифрование</p><p>• Не собираем данные • Открытый код</p><p>• Форум-группы • Стикеры • Звонки</p><br><p><strong>📜 Политика конфиденциальности:</strong></p><p style="font-size:0.75rem; color:#8E8E9A;">REVERS не собирает, не хранит и не передаёт третьим лицам никакие персональные данные.</p><br><p style="font-size:0.7rem; color:#6C6C7A; text-align:center;">© 2025 REVERS Messenger — GNU GPL v3</p></div><button id="closeAboutBtn" class="secondary">Закрыть</button></div>
      ...
      <div class="modal" id="addStickerModal"><h3>➕ Добавить стикер</h3><input type="file" id="stickerFileInput" accept="image/*"><p style="color:#8E8E9A; font-size:0.7rem; text-align:center;">Выберите изображение</p><div style="text-align:center;"><img id="stickerPreview" style="max-width:150px; max-height:150px; display:none; margin:10px auto;"></div><input type="text" id="stickerEmojiInput" placeholder="Эмодзи (например: 😊)"><button id="confirmStickerBtn">Добавить</button><button id="cancelStickerBtn" class="secondary">Отмена</button></div>
  ...
  _deleteMessage(msg, idx) {
    ...
    if (this.currentChat.type === 'saved') {
      REVERS.getChatHistory('me').then(h => { h.splice(idx, 1); REVERS.clearChatHistory('me'); h.forEach(x => REVERS.sendMessage('me', x.text)); });
    } else if (this.currentChat.type === 'group') {
      const h = REVERS.getGroupHistory(this.currentChat.id); h.splice(idx, 1);
    } else if (this.currentChat.type === 'channel') {
      const h = REVERS.getChannelHistory(this.currentChat.id); h.splice(idx, 1);
    } else {
      REVERS.getChatHistory(this.currentChat.id).then(h => { h.splice(idx, 1); REVERS.clearChatHistory(this.currentChat.id); h.forEach(x => REVERS.sendMessage(this.currentChat.id, x.text)); });
    }
  ...
  async _toggleVoiceRecord() {
    ...
    if (this.isRecording) {
      btn.textContent = '🎤 Голосовое'; this.isRecording = false;
      ...
      btn.textContent = '🔴 Запись...'; this.isRecording = true; this.voiceStartTime = Date.now(); this.voiceRecorder = REVERS.recordVoice();
    }
  }
  ...
  _sendMessage() {
    ...
    if (this._editingMessage) { this._editingMessage.text = text; this._editingMessage.edited = true; this._editingMessage = null; }
      ...
      if (g?.type === 'forum') {
        if (this._currentTopic) { groupManager.sendToTopic(this.currentChat.id, this._currentTopic, text); this._renderTopicMessages(); }
        else { const general = g.topics.find(t => t.id === 'general'); if (general) groupManager.sendToTopic(this.currentChat.id, 'general', text); else groupManager.sendGroupMessage(this.currentChat.id, text); this._renderMessages(this.currentChat); }
  ...
  _showChannelSettings() {
    ...
    document.getElementById('channelAvatarInput').addEventListener('change', (e) => {
      if (e.target.files[0]) {
        const r = new FileReader();
        r.onload = (ev) => { c.avatar = ev.target.result; document.getElementById('channelAvatarPreview').src = ev.target.result; REVERS._engine?.messageHandler?._saveChannels(); };
        r.readAsDataURL(e.target.files[0]);
      }
    });
    document.getElementById('deleteChannelSettingsBtn').addEventListener('click', () => {
  ...
  _openTopicsScreen() { if (!this.currentChat || this.currentChat.type !== 'group') return; document.getElementById('chatScreen').classList.add('hidden'); document.getElementById('topicsScreen').classList.remove('hidden'); document.getElementById('topicsScreen').querySelector('.current-chat-name').textContent = this.currentChat.name; this._renderTopicsList(); }
  ...
  _renderTopicsList() {
    ...
    topics.forEach(topic => {
      ...
      d.addEventListener('click', () => { this._currentTopic = topic.id; this._closeTopicsScreen(); this._openChat(this.currentChat); this._renderTopicMessages(); });
