...
class REVERSApp {
    ...
    document.getElementById('app').innerHTML = `
      ...
      <div class="modal" id="aboutModal">
        ...
        <div style="color:#EFEFEF; font-size:0.85rem; line-height:1.6;">
          ...
          <p>• Форум-группы с темами • Стикеры</p>
          <br>
          ...
          <p style="font-size:0.75rem; color:#8E8E9A;">
            ...
            Все сообщения передаются напрямую между устройствами (P2P).
          </p>
          <br>
  ...
  _bindEvents() {
    ...
    document.addEventListener('click', () => { document.getElementById('chatDropdown').classList.add('hidden'); document.getElementById('reactionPanel').classList.add('hidden'); document.getElementById('contextMenu').classList.add('hidden'); });
  ...
  _initWelcomeGroup() {
    ...
    if (key && REVERS.getAllChats().then(c => c.filter(x => x.id === key).length > 0)) return;
    const g = REVERS.createGroup('🖐 REVERS Welcome', 'forum');
    g.topics = [
      { id: 'general', name: '💬 Общий чат', closed: false, pinned: true, created: Date.now() },
      { id: 'rules', name: '📜 Правила', closed: true, pinned: true, created: Date.now() },
      { id: 'help', name: '❓ Помощь', closed: false, pinned: false, created: Date.now() }
    ];
    groupManager.groups.set(g.key, g);
    groupManager._save();
  ...
  _filterChats(query) { const all = REVERS.getAllChats().then(c => this._renderChatsList(query ? c.filter(x => x.name.toLowerCase().includes(query.toLowerCase())) : c)); }
  ...
  _showContextMenu(e, msg, idx) { e.preventDefault(); e.stopPropagation(); const menu = document.getElementById('contextMenu'); menu.style.top = (e.touches ? e.touches[0].clientY : e.clientY) + 'px'; menu.style.left = (e.touches ? e.touches[0].clientX : e.clientX) + 'px'; menu.classList.remove('hidden'); menu._target = { msg, idx }; }
  ...
  _replyToSelectedMessage() { const m = document.getElementById('contextMenu'); const { msg, idx } = m._target; this.replyTo = { text: msg.text, from: msg.from, idx }; document.getElementById('replyBarText').textContent = '↩️ ' + (msg.text || '').substring(0, 50); document.getElementById('replyBar').classList.remove('hidden'); document.getElementById('messageInput').focus(); m.classList.add('hidden'); }
  ...
  _deleteMessage(msg, idx) { if (!this.currentChat) return; let h = []; if (this.currentChat.type === 'saved') REVERS.getChatHistory('me').then(r => { h = r; h.splice(idx, 1); REVERS.clearChatHistory('me'); h.forEach(x => REVERS.sendMessage('me', x.text)); }); else if (this.currentChat.type === 'group') { h = REVERS.getGroupHistory(this.currentChat.id); h.splice(idx, 1); } else if (this.currentChat.type === 'channel') { h = REVERS.getChannelHistory(this.currentChat.id); h.splice(idx, 1); } else { REVERS.getChatHistory(this.currentChat.id).then(r => { h = r; h.splice(idx, 1); REVERS.clearChatHistory(this.currentChat.id); h.forEach(x => REVERS.sendMessage(this.currentChat.id, x.text)); }); } this._renderMessages(this.currentChat); }
  ...
  _showReactions(target) { const p = document.getElementById('reactionPanel'); const r = target.getBoundingClientRect(); p.style.top = (r.top - 50) + 'px'; p.style.left = (r.left + r.width/2 - 90) + 'px'; p.classList.remove('hidden'); p._target = target; }
  ...
  _openBrowser(url = 'https://www.startpage.com') { this._closeAllModals(); document.getElementById('browserModal').classList.add('active'); document.getElementById('browserUrlInput').value = url; this._navigateTo(url); }
  _navigateTo(url) { let u = url; if (!u.startsWith('http://') && !u.startsWith('https://')) u = 'https://' + u; document.getElementById('browserUrlInput').value = u; document.getElementById('browserFrame').src = u; }
  _navigateBrowser() { const u = document.getElementById('browserUrlInput').value.trim(); if (u) this._navigateTo(u); }
