// REVERS Messenger v3.2 — ИТОГОВЫЙ МОНОЛИТ
// Лицензия: GNU GPL v3
...
class REVERSApp {
  ...
  _buildDOM() {
    ...
    document.getElementById('app').innerHTML = `
      ...
      <div class="modal" id="accountModal"><h3>Аккаунт</h3><img id="modalAvatar" class="avatar-modal-img" src="${REVERS.getMyProfile().avatar || defaultAvatar}"><button id="changeAvatarBtn" class="secondary">Сменить</button><input type="file" id="modalAvatarInput" accept="image/*" style="display:none"><div id="myIdDisplay" style="color:white;font-family:monospace;font-size:0.7rem;word-break:break-all;">${REVERS.getMyId()}</div><button id="copyIdBtn" class="secondary">📋 ID</button><input type="text" id="nicknameInput" value="${REVERS.getMyProfile().name}"><button id="saveAccountBtn">Сохранить</button><button id="closeAccountBtn" class="secondary">Закрыть</button></div>
  ...
  _sendGift(recipientId, giftJson) { const gift = typeof giftJson === 'string' ? JSON.parse(giftJson) : giftJson; if (this.diamonds < gift.price) { alert(`Недостаточно алмазов! Нужно ${gift.price}💎, у вас ${this.diamonds}💎`); return; } this.diamonds -= gift.price; this._saveDiamonds(); if (this.currentChat?.id === recipientId) { REVERS.sendMessage(recipientId, `🎁 Подарок: ${gift.emoji} ${gift.name} от ${REVERS.getMyProfile().name}!`); this._renderMessages(this.currentChat); } this._closeAllModals(); }
