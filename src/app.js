// =============================================================================
// REVERS Messenger v3.3 — ПОЛНЫЙ APP.JS
// =============================================================================
// Лицензия: GNU GPL v3
// Разработчик: https://github.com/svet589
// =============================================================================
// ОПИСАНИЕ:
// Главный файл фронтенда. Отвечает за:
//   - Построение DOM (интерфейса)
//   - Навигацию между экранами
//   - Обработку всех событий (клики, ввод, свайпы)
//   - Отправку и отображение сообщений
//   - Управление стикерами, голосовыми, файлами
//   - Работу с группами, каналами, темами форума
//   - QR-коды, сканер, браузер
//   - Звонки, чат внутри звонка
//   - Профиль, настройки, память, политику
//   - Поиск по чатам и внутри чата
//   - Реакции, закреп, редактирование сообщений
//   - Черновики, отложенную отправку
//   - Алмазы и подарки
//   - Hotkeys, swipe back, back button
// =============================================================================
...
class REVERSApp {
  ...
  // ===========================================================================
  constructor() {
    // Централизованное состояние приложения (вместо разбросанных полей)
    this.state = {
      currentChat: null,           // Текущий открытый чат { id, type, name }
      currentTopic: null,          // Текущая тема форума (если открыта)
  ...
  async _init() {
    ...
    this._loadDiamonds();         // Загружаем алмазы из localStorage
  ...
  _bindEvents() {
    ...
    const sidebarItems = {
      ...
      'settingsMenuBtn': () => this._showScreen('settings'),  // Экран Настроек
      'privacyMenuBtn':  () => this._showPrivacy(),           // Политика конфиденциальности
      'aboutMenuBtn':    () => this._openModal('aboutModal')  // О нас
    };
    ...
    // =======================================================================
    // ЭКРАН АККАУНТА — кнопки внутри экрана
    // =======================================================================
    ...
    document.getElementById('accountProfileBtn')?.addEventListener('click', () => this._openProfileScreen());
    document.getElementById('accountStorageBtn')?.addEventListener('click', () => this._showStorageInfo());
    document.getElementById('accountPrivacyBtn')?.addEventListener('click', () => this._showPrivacy());
    document.getElementById('accountSupportBtn')?.addEventListener('click', () => this._showSupport());

    // =======================================================================
    // ЭКРАН НАСТРОЕК — кнопки внутри экрана
    // =======================================================================
    ...
    document.getElementById('settingsStorageBtn')?.addEventListener('click', () => this._showStorageInfo());
    document.getElementById('settingsExportBtn')?.addEventListener('click', () => this._exportData());
    document.getElementById('settingsFoldersBtn')?.addEventListener('click', () => this._showFolders());

    // =======================================================================
    // ДРОПДАУНЫ ЧАТА — пригласить, поиск, закреп, безопасность, звонки
    // =======================================================================
    ...
    document.getElementById('callBtn')?.addEventListener('click', () => {
      if (this.state.currentChat && REVERS) REVERS.startCall(this.state.currentChat.id, true);
    });
    document.getElementById('audioCallBtn')?.addEventListener('click', () => {
      if (this.state.currentChat && REVERS) REVERS.startCall(this.state.currentChat.id, false);
    });
    document.getElementById('groupCallBtn')?.addEventListener('click', async () => {
      if (this.state.currentChat && REVERS) await REVERS.startGroupCall(this.state.currentChat.id, true);
    });
  ...
  // Показать экран по имени (chats, chat, account, settings, topics)
  _showScreen(name) {
  ...
  // Обновить данные на экране аккаунта
  _updateAccountScreen() {
    ...
    if (avatar) avatar.src = this.state.myProfile?.avatar || '';
    ...
    // Статус онлайн (зелёный если есть подключённые пиры)
    if (online) {
  ...
  // Открыть профиль (в модалке)
  _openProfileScreen() {
    const content = document.getElementById('groupSettingsContent');
    if (content) {
      content.innerHTML = `
        <div style="text-align:center;">
          <img src="${this.state.myProfile?.avatar || ''}" style="width:80px;height:80px;border-radius:50%;object-fit:cover;">
          <h3 style="color:white;">${this.state.myProfile?.name || 'User'}</h3>
          <p style="color:#8E8E9A;font-size:0.7rem;">ID: ${this.state.myId}</p>
          <p style="color:#EFEFEF;">${localStorage.getItem('revers_bio') || ''}</p>
        </div>
      `;
    }
    const modal = document.getElementById('groupSettingsModal');
    if (modal) {
      modal.querySelector('h3').textContent = '👤 Профиль';
  ...
  _showStorageInfo() {
    ...
    if (content) {
      content.innerHTML = `
        <div style="text-align:center;padding:20px;">
          <p style="color:white;font-size:1.2rem;">💾 ${mb} MB</p>
          <p style="color:#8E8E9A;font-size:0.8rem;">Использовано localStorage</p>
          <button id="clearStorageBtn" style="background:#E63946;border:none;padding:10px;border-radius:20px;color:white;width:100%;cursor:pointer;margin-top:20px;">🗑️ Очистить все данные</button>
          <button id="clearCacheBtn" style="background:#2A2A3A;border:none;padding:10px;border-radius:20px;color:white;width:100%;margin-top:8px;cursor:pointer;">🧹 Очистить кэш</button>
        </div>
      `;
      ...
      // Кнопка очистки кэша (черновики, закрепы, relay-кэш)
      document.getElementById('clearCacheBtn')?.addEventListener('click', () => {
        ...
        this._showToast(`🧹 Очищено ${keys.length} записей кэша`);
    ...
    const modal = document.getElementById('groupSettingsModal');
    if (modal) {
      modal.querySelector('h3').textContent = '💾 Память';
      this._openModal('groupSettingsModal');
    }
  }
  ...
  _showPrivacy() {
    ...
    if (content) {
      content.innerHTML = `
        <div style="color:#EFEFEF;font-size:0.85rem;line-height:1.6;padding:10px;">
          <h4 style="color:#E63946;">Политика конфиденциальности REVERS</h4>
          <p>REVERS — децентрализованный P2P-мессенджер.</p>
          <p><b>Мы НЕ собираем:</b></p>
          <ul style="padding-left:16px;">
            <li>Номера телефонов</li>
            <li>Список контактов</li>
            <li>Историю переписки</li>
            <li>Метаданные звонков</li>
          </ul>
          <p><b>Все данные хранятся ТОЛЬКО на вашем устройстве.</b></p>
          <p>Сообщения шифруются X25519 + ML-KEM-768 + ChaCha20-Poly1305.</p>
          <p>Никакие третьи лица не имеют доступа к вашим данным.</p>
        </div>
      `;
    }
    const modal = document.getElementById('groupSettingsModal');
    if (modal) {
      modal.querySelector('h3').textContent = '🔒 Политика конфиденциальности';
      this._openModal('groupSettingsModal');
    }
  }
  ...
  _showSupport() {
    ...
    if (content) {
      content.innerHTML = `
        <div style="color:#EFEFEF;font-size:0.85rem;line-height:1.6;padding:10px;">
          <h4 style="color:#E63946;">❓ Часто задаваемые вопросы</h4>
          <p><b>Как добавить друга?</b><br>Нажмите ➕ и введите ID друга. Или используйте QR-код.</p>
          <p><b>Почему сообщения не доходят?</b><br>Убедитесь что собеседник в сети и у вас есть интернет.</p>
          <p><b>Как работают групповые звонки?</b><br>До 10 участников, P2P, без серверов.</p>
          <p><b>Куда писать если баг?</b><br>GitHub Issues: github.com/svet589/REVERS2_0</p>
        </div>
      `;
    }
    const modal = document.getElementById('groupSettingsModal');
    if (modal) {
      modal.querySelector('h3').textContent = '❓ Поддержка';
      this._openModal('groupSettingsModal');
    }
  }
  ...
  _showFolders() {
    ...
    if (content) {
      content.innerHTML = `
        <div style="color:#EFEFEF;padding:10px;">
          <p>📂 Папки для чатов</p>
          <p style="color:#8E8E9A;">В разработке</p>
        </div>
      `;
    }
    const modal = document.getElementById('groupSettingsModal');
    if (modal) {
      modal.querySelector('h3').textContent = '📂 Папки';
      this._openModal('groupSettingsModal');
    }
  }
  ...
  // Мгновенно добавить сообщение в DOM (до подтверждения от ядра)
  _appendMessageToUI(msg, isOutgoing) {
    ...
    div.innerHTML = `
      <div class="bubble">
        ...
        ${isOutgoing ? '<span style="font-size:0.6rem;color:#8E8E9A;">✓</span>' : ''}
      </div>
    `;
  ...
  async _renderMessages(chat) {
    ...
    try {
      if (chat.type === 'saved') {
        history = REVERS ? (await REVERS.getChatHistory('me') || []) : [];
    ...
    for (const msg of history) {
      ...
      if (msg.type === 'file' && msg.fileName) {
        html += `<div class="file-attachment" onclick="window.REVERSApp._openMedia('${msg.fileData || ''}', '${msg.fileType || ''}')">📄 ${this._escapeHtml(msg.fileName)}</div>`;
      }
  ...
  // Превью стикера при выборе файла
  _previewSticker(e) {
  ...
  _filterChats(query) {
    ...
    REVERS.getAllChats().then(all => {
      this._renderChatsList(
        all.filter(c => (c.name || c.id).toLowerCase().includes(query.toLowerCase()))
      );
    });
  }
  ...
  _renderTopicsList() {
    ...
    for (const t of topics) {
      ...
      div.addEventListener('click', () => {
        ...
        const chatName = document.getElementById('chatName');
        if (chatName) chatName.textContent = t.name;
  ...
  _showGroupSettings() {
    ...
    const modal = document.getElementById('groupSettingsModal');
    if (modal) { modal.querySelector('h3').textContent = '⚙️ Настройки группы'; this._openModal('groupSettingsModal'); }
  }
  ...
  _editSelectedMessage() {
    ...
    if (menu?._target) {
      ...
      const input = document.getElementById('messageInput');
      if (input) input.value = menu._target.msg.text || '';
  ...
  async _deleteMessage(msg, idx) {
    ...
    if (this.state.currentChat?.type === 'saved') {
      ...
      for (const m of history) await REVERS.sendMessage('me', m.text);
    }
    await this._renderMessages(this.state.currentChat);
  }
  ...
  _openConnectModal() {
    ...
    if (output) {
      ...
      setTimeout(() => { if (output) output.value = 'Сигнал готов'; }, 500);
    }
