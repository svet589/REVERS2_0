// REVERS Messenger v1.3.1 — фикс 12 багов
// Лицензия: GNU GPL v3
...
class REVERSApp {
    ...
    document.getElementById('app').innerHTML = `
      ...
      <div id="chatScreen" class="screen hidden">
        ...
        <div class="input-panel">
          <button class="sticker-btn" id="stickerToggleBtn" style="font-size:1.3rem; padding:4px 6px;">😊</button>
          <button class="sticker-btn" id="voiceRecordBtn" style="font-size:1.2rem; padding:4px 6px;">🎤</button>
      ...
      <div class="modal" id="addContactModal"><h3>➕ Добавить контакт</h3><input type="text" id="addContactIdInput" placeholder="ID собеседника"><button id="addContactConfirmBtn">Добавить</button><button id="addContactCloseBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="connectModal"><h3>🔗 Подключиться</h3><input type="text" id="peerIdInput" placeholder="ID собеседника"><input type="text" id="signalInput" placeholder="Или вставьте сигнал (JSON)"><button id="connectSendBtn">Подключиться</button><div style="text-align:center; margin-top:8px;"><small style="color:#8E8E9A">Мой сигнал:</small><textarea id="mySignalOutput" readonly style="width:100%; height:60px; background:#0F0F12; color:#2A9D8F; border:none; border-radius:12px; padding:8px; font-size:0.65rem; resize:none; margin-top:4px;"></textarea><button id="copySignalBtn" class="secondary" style="margin-top:4px;">📋 Копировать сигнал</button></div><button id="closeConnectBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="qrModal"><h3>🔳 Мой QR-код</h3><div style="text-align:center; background:white; padding:20px; border-radius:20px;"><canvas id="qrCanvas" style="width:200px; height:200px;"></canvas></div><div style="background:#1A1A23; border-radius:12px; padding:8px; margin-top:8px; word-break:break-all;"><code style="color:#2A9D8F; font-size:0.6rem;" id="inviteLink"></code></div><button id="copyInviteLinkBtn" class="secondary" style="margin-top:4px;">📋 Копировать ссылку</button><button id="shareInviteBtn" class="secondary" style="margin-top:4px;">📤 Поделиться</button><button id="closeQrBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="scannerModal"><h3>📷 Сканировать QR</h3><div id="scannerContainer" style="width:100%; border-radius:16px; overflow:hidden;"></div><p style="color:#8E8E9A; font-size:0.7rem; text-align:center;">Наведите камеру на QR-код</p><button id="stopScannerBtn" class="secondary">Отмена</button></div>
      <div class="modal" id="browserModal"><h3>🌐 Браузер</h3><input type="text" id="browserUrlInput" placeholder="Введите адрес..."><button id="browserGoBtn">Открыть</button><button id="closeBrowserBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="accountModal"><h3>Аккаунт</h3><div style="text-align:center"><img id="modalAvatar" class="avatar-modal-img" src="${REVERS.getMyProfile().avatar || defaultAvatar}"><button id="changeAvatarBtn" class="secondary" style="margin-top:8px;">Сменить аватар</button><input type="file" id="modalAvatarInput" accept="image/*" style="display:none"></div><div style="text-align:center; word-break:break-all;"><small style="color:#8E8E9A">Мой ID:</small><div style="color:white; font-family:monospace; font-size:0.7rem;" id="myIdDisplay">${REVERS.getMyId()}</div><button id="copyIdBtn" class="secondary" style="margin-top:4px;">📋 Копировать</button></div><input type="text" id="nicknameInput" placeholder="Имя..." value="${REVERS.getMyProfile().name}"><button id="saveAccountBtn">Сохранить</button><button id="closeAccountBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="groupsModal"><h3>👥 Группы</h3><div id="groupsList"></div><button id="createGroupBtn">➕ Создать группу</button><button id="closeGroupsBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="createGroupModal"><h3>Создать группу</h3><input type="text" id="groupNameInput" placeholder="Название группы"><div class="setting-row"><span class="setting-label">Тип:</span><select id="groupTypeSelect" style="background:#2A2A3A; border:none; border-radius:12px; padding:8px; color:white; font-size:0.9rem;"><option value="chat">💬 Чат</option><option value="forum">📂 Форум</option></select></div><button id="confirmGroupBtn">Создать</button><button id="cancelGroupBtn" class="secondary">Отмена</button></div>
      <div class="modal" id="channelsModal"><h3>📢 Каналы</h3><div id="channelsList"></div><button id="createChannelBtn">➕ Создать канал</button><button id="closeChannelsBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="createChannelModal"><h3>Создать канал</h3><input type="text" id="channelNameInput" placeholder="Название канала"><button id="confirmChannelBtn">Создать</button><button id="cancelChannelBtn" class="secondary">Отмена</button></div>
      ...
      <div class="modal" id="createTopicModal"><h3>📂 Новая тема</h3><input type="text" id="topicNameInput" placeholder="Название темы"><div class="setting-row"><span class="setting-label">🔒 Закрытая тема</span><div id="topicClosedToggle" class="toggle-switch"></div></div><button id="confirmTopicBtn">Создать</button><button id="cancelTopicBtn" class="secondary">Отмена</button></div>
      <div class="modal" id="settingsModal"><h3>⚙️ Настройки</h3><div class="setting-row"><span class="setting-label">🌓 Тёмная тема</span><div id="themeToggle" class="toggle-switch active"></div></div><div class="setting-row"><span class="setting-label">🔊 Звук</span><div id="soundToggle" class="toggle-switch active"></div></div><button id="closeSettingsBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="aboutModal"><h3>ℹ️ О REVERS Messenger</h3><div style="color:#EFEFEF; font-size:0.85rem;"><p><strong>REVERS v1.3.1</strong></p><p>P2P мессенджер с полной анонимностью.</p><p>• Никаких серверов • Пост-квантовое шифрование</p><br><p style="font-size:0.7rem; color:#6C6C7A; text-align:center;">© 2025 REVERS Messenger — GNU GPL v3</p></div><button id="closeAboutBtn" class="secondary">Закрыть</button></div>
      <div class="modal" id="createStickerPackModal"><h3>🎨 Создать стикерпак</h3><input type="text" id="stickerPackNameInput" placeholder="Название пакета"><button id="confirmStickerPackBtn">Создать</button><button id="cancelStickerPackBtn" class="secondary">Отмена</button></div>
      <div class="modal" id="addStickerModal"><h3>➕ Добавить стикер</h3><input type="file" id="stickerFileInput" accept="image/*"><div style="text-align:center;"><img id="stickerPreview" style="max-width:120px; max-height:120px; display:none; margin:10px auto;"></div><input type="text" id="stickerEmojiInput" placeholder="Эмодзи (например: 😊)"><button id="confirmStickerBtn">Добавить</button><button id="cancelStickerBtn" class="secondary">Отмена</button></div>
      <div class="image-viewer" id="imageViewer"><img id="fullscreenImage" src=""></div>
      <div class="video-player hidden" id="videoPlayer"><video id="videoPlayerEl" controls style="width:100%; height:100%;"></video><button class="close-player" id="closeVideoPlayer">✖</button></div>
    `;
  }
  ...
  _bindBackButton() {
    document.addEventListener('backbutton', (e) => {
      ...
      if (document.querySelector('.modal.active')) {
        ...
        this._currentTopic = null;
        this._openChat(this.currentChat);
  ...
  _bindEvents() {
    ...
    document.getElementById('backBtn').addEventListener('click', () => {
      if (this._currentTopic) { this._currentTopic = null; this._openChat(this.currentChat); }
      else this._goToChats();
    });
    ...
    document.getElementById('closeVideoPlayer').addEventListener('click', () => { document.getElementById('videoPlayer').classList.add('hidden'); document.getElementById('videoPlayerEl').pause(); });
  }
  ...
  _initWelcomeGroup() {
    ...
    const existing = groupManager.groups.get(key);
    if (key && existing) return;
    if (key) localStorage.removeItem('revers_welcome');
    const g = REVERS.createGroup('🖐 REVERS Welcome', 'forum');
    const grp = groupManager.groups.get(g.key);
    if (grp) {
      grp.topics = [
  ...
  _showChatMenu() {
    ...
    const id = type === 'saved' ? 'savedDropdown' : type === 'group' ? 'groupDropdown' : type === 'channel' ? 'channelDropdown' : 'contactDropdown';
    const menu = document.getElementById(id);
    if (menu) menu.classList.remove('hidden');
  }

  _toggleChatSearch(show = true) { const b = document.getElementById('searchChatsBar'); b.classList.toggle('hidden', !show); if (show) document.getElementById('searchChatsInput').focus(); else { document.getElementById('searchChatsInput').value = ''; this._renderChatsList(); } }
  ...
  _toggleSearchInChat(show = true) { const b = document.getElementById('searchInChatBar'); b.classList.toggle('hidden', !show); if (show) document.getElementById('searchInChatInput').focus(); else { document.getElementById('searchInChatInput').value = ''; this._clearSearchHighlights(); } }
  ...
  _showContextMenu(e, msg, idx) {
    ...
    const menu = document.getElementById('contextMenu'); menu.classList.remove('hidden');
    const x = Math.min((e.touches ? e.touches[0].clientX : e.clientX), window.innerWidth - 190);
    menu.style.left = x + 'px'; menu.style.top = (e.touches ? e.touches[0].clientY : e.clientY) + 'px';
  ...
  _showReactions(target) { const p = document.getElementById('reactionPanel'); const r = target.getBoundingClientRect(); p.style.top = (r.top - 50) + 'px'; p.style.left = Math.min(r.left + r.width/2 - 90, window.innerWidth - 200) + 'px'; p.classList.remove('hidden'); p._target = target; this._selectedMsg = target; }
  
  _addReaction(emoji) {
    const target = document.getElementById('reactionPanel')._target || this._selectedMsg;
    if (!target) { document.getElementById('reactionPanel').classList.add('hidden'); return; }
    const bubble = target.querySelector('.bubble');
    if (!bubble) { document.getElementById('reactionPanel').classList.add('hidden'); return; }
    const existing = bubble.querySelector('.reactions');
    if (existing) { if (!existing.textContent.includes(emoji)) existing.textContent += emoji; }
    else { const r = document.createElement('div'); r.className = 'reactions'; r.textContent = emoji; bubble.appendChild(r); }
  ...
  async _openInBrowser() { let url = document.getElementById('browserUrlInput').value.trim(); if (!url) return; if (!url.startsWith('http')) url = 'https://' + url; this._closeAllModals(); try { const { Browser } = await import('@capacitor/browser'); await Browser.open({ url }); } catch(e) { window.open(url, '_system'); } }
  ...
  _playVideo(url) {
    ...
    const el = document.getElementById('videoPlayerEl');
    el.src = url;
  ...
  async _toggleVoiceRecord() {
    ...
    if (this.isRecording) {
      ...
      if (this.voiceRecorder) {
        const r = await this.voiceRecorder;
        if (r?.stop) {
          const audio = await new Promise(resolve => {
            r.recorder.onstop = () => { setTimeout(() => { const blob = new Blob(r.chunks || [], { type: 'audio/webm' }); const reader = new FileReader(); reader.onload = () => resolve(reader.result); reader.readAsDataURL(blob); }, 100); };
            r.stop();
          });
          if (audio && this.currentChat) { const dur = Math.round((Date.now() - this.voiceStartTime) / 1000); const tid = this.currentChat.type === 'saved' ? 'me' : this.currentChat.id; REVERS.sendVoice(tid, audio, dur || 1); this._renderMessages(this.currentChat); this._renderChatsList(); }
        }
        this.voiceRecorder = null;
      }
    } else {
      ...
      btn.textContent = '🔴'; this.isRecording = true; this.voiceStartTime = Date.now(); this.voiceRecorder = REVERS.recordVoice();
    }
  }
  ...
  _sendMessage() {
    ...
    if (this._editingMessage) { this._editingMessage.text = text; this._editingMessage.edited = true; this._editingMessage = null; this._renderMessages(this.currentChat); }
    else if (this.currentChat.type === 'group') {
      const g = groupManager.groups.get(this.currentChat.id);
      if (g?.type === 'forum' && this._currentTopic) { groupManager.sendToTopic(this.currentChat.id, this._currentTopic, text); this._renderTopicMessages(); }
      else if (g?.type === 'forum') { groupManager.sendToTopic(this.currentChat.id, 'general', text); this._renderMessages(this.currentChat); }
      else { REVERS.sendGroupMessage(this.currentChat.id, text); this._renderMessages(this.currentChat); }
    }
    else if (this.currentChat.type === 'saved') { REVERS.sendMessage('me', text); this._renderMessages(this.currentChat); }
    else if (this.currentChat.type === 'channel') { REVERS.sendChannelMessage(this.currentChat.id, text); this._renderMessages(this.currentChat); }
    else { REVERS.sendMessage(this.currentChat.id, text); this._renderMessages(this.currentChat); }
    input.value = ''; this.replyTo = null; document.getElementById('replyBar').classList.add('hidden'); this._renderChatsList();
  }
  ...
  _showGroupSettings() {
    ...
    c.innerHTML = `
      ...
      <div style="text-align:center; margin:8px 0;"><p style="color:#8E8E9A; font-size:0.8rem;">Аватар группы:</p><img id="groupAvatarPreview" class="avatar-modal-img" src="${g.avatar || 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Ccircle fill=%22%232A2A3A%22 cx=%2250%22 cy=%2250%22 r=%2250%22/%3E%3Ctext x=%2250%22 y=%2267%22 text-anchor=%22middle%22 fill=%22%23E63946%22 font-size=%2240%22%3E👥%3C/text%3E%3C/svg%3E'}" style="width:60px; height:60px; margin:0 auto; display:block;"><button id="changeGroupAvatarBtn" class="secondary" style="margin-top:8px;">Сменить аватар</button><input type="file" id="groupAvatarInput" accept="image/*" style="display:none"></div>
    ...
    document.getElementById('groupAvatarInput').addEventListener('change', (e) => { if (e.target.files[0]) { const r = new FileReader(); r.onload = (ev) => { groupManager.setGroupAvatar(this.currentChat.id, ev.target.result); document.getElementById('groupAvatarPreview').src = ev.target.result; }; r.readAsDataURL(e.target.files[0]); } });
    this._openModal('groupSettingsModal');
  }

  _showChannelSettings() {
    if (!this.currentChat || this.currentChat.type !== 'channel') return;
    const ch = REVERS._engine?.messageHandler?.channels?.get(this.currentChat.id);
    if (!ch) return;
    const c = document.getElementById('groupSettingsContent');
    c.innerHTML = `
      <div class="setting-row"><span class="setting-label">Название:</span><span style="color:#8E8E9A;">${this._esc(ch.name)}</span></div>
      <div style="text-align:center; margin:8px 0;"><p style="color:#8E8E9A; font-size:0.8rem;">Аватар канала:</p><img id="channelAvatarPreview" class="avatar-modal-img" src="${ch.avatar || 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Ccircle fill=%22%232A2A3A%22 cx=%2250%22 cy=%2250%22 r=%2250%22/%3E%3Ctext x=%2250%22 y=%2267%22 text-anchor=%22middle%22 fill=%22%23E63946%22 font-size=%2240%22%3E📢%3C/text%3E%3C/svg%3E'}" style="width:60px; height:60px; margin:0 auto; display:block;"><button id="changeChannelAvatarBtn" class="secondary" style="margin-top:8px;">Сменить аватар</button><input type="file" id="channelAvatarInput" accept="image/*" style="display:none"></div>
      <button id="deleteChannelSettingsBtn" style="background:#E63946; border:none; padding:8px; border-radius:12px; color:white; width:100%; margin-top:4px;">🗑️ Удалить канал</button>
    `;
    document.getElementById('changeChannelAvatarBtn').addEventListener('click', () => document.getElementById('channelAvatarInput').click());
    document.getElementById('channelAvatarInput').addEventListener('change', (e) => { if (e.target.files[0]) { const r = new FileReader(); r.onload = (ev) => { ch.avatar = ev.target.result; document.getElementById('channelAvatarPreview').src = ev.target.result; REVERS._engine?.messageHandler?._saveChannels(); }; r.readAsDataURL(e.target.files[0]); } });
    document.getElementById('deleteChannelSettingsBtn').addEventListener('click', () => { if (confirm('Удалить канал?')) { REVERS._engine?.messageHandler?.channels?.delete(this.currentChat.id); REVERS._engine?.messageHandler?._saveChannels(); this._goToChats(); this._closeAllModals(); } });
  ...
  _renderTopicsList() {
    ...
    topics.forEach(topic => {
      ...
      d.addEventListener('click', () => { this._currentTopic = topic.id; document.getElementById('topicsScreen').classList.add('hidden'); document.getElementById('chatScreen').classList.remove('hidden'); document.getElementById('chatName').textContent = topic.name; this._renderTopicMessages(); });
      container.appendChild(d);
    });
  }
  _createTopic() { const n = document.getElementById('topicNameInput').value.trim(); if (!n || !this.currentChat) return; const closed = document.getElementById('topicClosedToggle').classList.contains('active'); const topic = groupManager.addTopic(this.currentChat.id, n); if (topic && closed) groupManager.toggleCloseTopic(this.currentChat.id, topic.id); this._closeAllModals(); this._renderTopicsList(); document.getElementById('topicNameInput').value = ''; document.getElementById('topicClosedToggle').classList.remove('active'); }
  ...
  _openChat(chat) {
    ...
    document.querySelectorAll('.chat-dropdown').forEach(d => d.classList.add('hidden'));
    document.getElementById('reactionPanel').classList.add('hidden'); document.getElementById('contextMenu').classList.add('hidden');
    this._togglePinnedMessage(); this.replyTo = null; this._editingMessage = null; document.getElementById('replyBar').classList.add('hidden');
  }
  ...
  async _renderMessages(chat) {
    ...
    history.forEach((msg, idx) => {
      ...
      if (this.replyTo && this.replyTo.idx === idx) html += '<div class="reply-context">↩️ ' + this._esc((this.replyTo.text || '').substring(0, 50)) + '</div>';
      ...
      if (msg.type === 'sticker' && msg.fileData) html += '<img src="' + msg.fileData + '" style="max-width:120px; max-height:120px; margin-top:4px;">';
      if (msg.type === 'file' && msg.fileName) {
        ...
        html += '<div class="file-attachment" onclick="window.REVERSApp._openMedia(\'' + (msg.fileData || '') + '\', \'' + (msg.fileType || '') + '\')">📄 ' + this._esc(msg.fileName) + ' (' + this._formatSize(msg.fileSize) + ')</div>';
        if (isImage && msg.fileData) html += '<img src="' + msg.fileData + '" class="file-preview-img" onclick="event.stopPropagation(); window.REVERSApp._openMedia(\'' + msg.fileData + '\', \'' + msg.fileType + '\')">';
        if (isVideo && msg.fileData) html += '<div class="file-attachment" onclick="window.REVERSApp._openMedia(\'' + msg.fileData + '\', \'' + msg.fileType + '\')">🎬 Нажмите для воспроизведения</div>';
  ...
  _openMedia(data, type) {
    ...
    if (type?.startsWith('image/')) {
      document.getElementById('fullscreenImage').src = data;
  ...
  _deleteCurrentChat() { if (!this.currentChat || this.currentChat.type === 'saved') return; if (!confirm('Удалить?')) return; if (this.currentChat.type === 'group') { groupManager.deleteGroup(this.currentChat.id); } else if (this.currentChat.type === 'channel') { REVERS._engine?.messageHandler?.channels?.delete(this.currentChat.id); REVERS._engine?.messageHandler?._saveChannels(); } this._goToChats(); }
