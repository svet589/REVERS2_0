...
class REVERSApp {
  ...
  async _startScanner() {
    ...
    try {
      ...
      scannerContainer.innerHTML = '<div style="text-align:center; padding:40px; color:#E63946;"><p>❌ Нет доступа к камере</p><p style="font-size:0.7rem; color:#8E8E9A;">Разрешите доступ в настройках</p></div>';
    }
  }
      ...
      if (this.voiceRecorder) {
        ...
        if (result && result.stop) {
          const audioBase64 = await new Promise(resolve => {
            const origStop = result.stop;
            result.recorder.onstop = () => {};
            result.recorder.onstop = () => {
              const chunks = [];
              result.recorder.ondataavailable = e => chunks.push(e.data);
              setTimeout(() => {
  ...
  _updateConnectionStatus(connected) {
    const led = document.getElementById('statusLed'); const text = document.getElementById('statusText');
    if (connected) { led.classList.add('green'); text.textContent = 'P2P Online'; }
    else { led.classList.remove('green'); text.textContent = 'Оффлайн'; }
  }
